...
- 수치를 설명할 때 숫자만 나열하지 말고 의미를 풀어써. 예) "은퇴까지 시간이 충분해서 (+7.0%p)", "자금 여력이 부족해서 (-3.2%p)"
- 마지막 줄은 한 줄 요약으로 마무리
- 전체 답변 5줄 이내
`;
...
function buildPersonaContext(ctx) {
  ...
  const lv   = (v) => (v ?? 0) >= 0.7 ? '높음' : (v ?? 0) >= 0.4 ? '보통' : '낮음';
...
============================================================
■ 페르소나: ${persona?.name ?? ''}
■ 위험회피계수(A): ${f2(A)} (낮을수록 공격적, 1~8 범위)
■ 위험자산 비중(y*): ${pct(yStar)} — 전체 적립금 중 주식형 ETF 등에 투자하는 비중 (법적 상한 70%)
■ 종합 투자 성향 점수: ${pct(riskScore)} (높을수록 공격적 투자에 유리한 상황)

■ 성향 구성 요소 (각 항목이 높을수록 공격적 투자에 유리)
   - 은퇴까지 남은 시간: ${pct(timeScore)} (${lv(timeScore)}) — 시간이 길수록 손실 회복 여유 있음
   - 자금·소득 여력: ${pct(capitalScore)} (${lv(capitalScore)}) — 투자 여유 자금과 소득 안정성 종합
   - 직업 안정성: ${pct(jobScore)} (${lv(jobScore)}) — 안정적일수록 단기 손실 버틸 수 있음
   - 가족 부양 부담: ${pct(familyScore)} (${lv(familyScore)}) — 부양 책임이 적을수록 위험 감내 가능

■ 손실 위험 지표 (2016~2025년 실제 월별 수익률 데이터 기반)
   - 나쁜 달 평균 손실 (100번 중 5번 수준): ${f1(cvar95)}%
   - 최악의 달 평균 손실 (100번 중 1번 수준): ${f1(cvar99)}%
   - 하방 위험 대비 수익 비율 (소르티노): ${f2(sortino)}

■ 비중 결정 요인별 기여도 (위험자산 비중에 얼마나 영향 줬는지)
${shapLines}
